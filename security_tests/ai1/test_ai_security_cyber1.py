...
The failed tests intentionally express the secure behaviour that was expected.
They demonstrate grounding and prompt-injection weaknesses in the tested AI 1
implementation.
"""
...
from app.services.generation import QuestionGenerator, rejection_reasons
...
def test_legitimate_grounded_question_is_accepted():
    ...
    reasons = rejection_reasons(make_draft(), transfer_chunks())
...
def test_unrelated_question_with_real_citation_is_rejected():
    ...
    reasons = rejection_reasons(draft, transfer_chunks())
...
def test_wrong_answer_with_real_citation_is_rejected():
    ...
    draft = make_draft(correct_option=1)
...
def test_malicious_document_instruction_is_not_valid_grounding():
    ...
    reasons = rejection_reasons(draft, malicious_chunks())

    assert reasons, "Malicious document instructions were accepted as grounding."


class FakeChatClient:
    """Simulates a model following the malicious instruction in the document."""
...
async def test_generator_output_following_prompt_injection_is_rejected():
    ...
    outcome = await generator._draft(malicious_chunks())

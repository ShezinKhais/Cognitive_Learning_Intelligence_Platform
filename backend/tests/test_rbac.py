...
def test_admin_route_requires_terms_but_not_student_monitoring_consent(
    client: TestClient,
) -> None:
    token = _login(
        client,
...
def test_student_and_lecturer_cannot_upload_a_timetable(
    client: TestClient,
) -> None:
...
def test_admin_can_reach_timetable_and_roster_handlers(
    client: TestClient,
) -> None:
    token = _login(
        client,
...
def test_admin_routes_without_a_token_return_401(
    client: TestClient,
) -> None:
    response = client.post(
        "/api/v1/admin/roster",
